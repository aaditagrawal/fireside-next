    ...
    if (feed.publisher?.name) {
      ...
      if (Array.isArray(publisherResult) && publisherResult.length > 0) {
        ...
        const newPublisher = await executeQuery({
          ...
          values: [
            feed.publisher.name,
            feed.publisher.url || feed.link || null,
            feed.publisher.logo || null,
          ],
        });
      ...
      try {
        ...
        if (item.categories && item.categories.length > 0) {
          for (const categoryName of item.categories) {
            ...
            // Since we don't have a dedicated FeedItemCategories table, we'll associate
            // categories with the feed and possibly track this in User_FeedItems metadata
            await executeQuery({
              query:
                "INSERT IGNORE INTO Feed_Categories (FeedID, CategoryID) VALUES (?, ?)",
              values: [feedId, categoryId],
