...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (type === "read") {
      // First, check if a User_FeedItems record exists
      ...
      // Also update the global read status (for backward compatibility)
      await executeQuery({
    ...
    if (type === "save" || shouldSave) {
      ...
      if (Array.isArray(existingRecord) && existingRecord.length > 0) {
        ...
        await executeQuery({
          query:
            "INSERT INTO User_FeedItems (UserID, ItemID, IsRead, IsSaved, LastInteractionAt) VALUES (?, ?, 0, 1, NOW())",
          values: [user.id, itemId],
        });
      }
    }
